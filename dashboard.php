...
<body>
    <div class="container">
        ...
        <div class="main-content" id="main-content">
            ...
            <!-- Pending Approvals -->
            <?php 
                $pending_students = $conn->query("SELECT first_name, last_name, created_at FROM students WHERE archived = 0 AND status = 'PENDING' ORDER BY id DESC LIMIT 5");
                $pending_count_row = $conn->query("SELECT COUNT(*) AS cnt FROM students WHERE archived = 0 AND status = 'PENDING'");
                $pending_total = $pending_count_row ? intval(($pending_count_row->fetch_assoc()['cnt']) ?? 0) : 0;
            ?>
            <div class="quick-actions">
                ...
                <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:15px; gap:10px; flex-wrap:wrap;">
                    <div style="display:flex; align-items:center; gap:10px; font-weight:700; color:#1B5E20;">
                        <span style="display:inline-flex; align-items:center; gap:8px; background:#FFF8E1; color:#8D6E63; border:1px solid #FFE082; padding:6px 12px; border-radius:999px;">
                ...
                <div class="actions-grid" style="grid-template-columns: 1fr;">
                    <div class="activity-card" style="margin:0;">
                        <?php if ($pending_students && $pending_students->num_rows > 0): ?>
                            <?php while($p = $pending_students->fetch_assoc()): ?>
                                <div class="activity-item">
                                    <div class="activity-avatar">
                                        <?php echo strtoupper(substr($p['first_name'] ?? 'N', 0, 1)); ?>
                                    </div>
                                    <div class="activity-content">
                                        <div class="activity-name">
                                            <?php echo htmlspecialchars(($p['first_name'] ?? '') . ' ' . ($p['last_name'] ?? '')); ?>
                                        </div>
                                        <div class="activity-detail">Awaiting approval</div>
                                        <div class="activity-time">
                                            <?php 
                                                $ts = isset($p['created_at']) ? strtotime($p['created_at']) : false;
                        ...
                        <?php else: ?>
                            <div class="activity-item">
                                <div class="activity-content">
                                    <div class="activity-name">No pending applications</div>
                                    <div class="activity-detail">You're all caught up!</div>
                                </div>
                            </div>
                        <?php endif; ?>
                    </div>
